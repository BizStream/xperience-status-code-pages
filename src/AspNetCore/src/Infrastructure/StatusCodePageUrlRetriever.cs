...
public class StatusCodePageUrlRetriever : IStatusCodePageUrlRetriever
{
    ...
    /// <inheritdoc />
    public virtual async Task<PageUrl?> RetrieveAsync( int statusCode, CancellationToken cancellation = default )
    {
        var node = await statusCodePageRetriever.RetrieveAsync( statusCode, cancellation );
        if( node is null )
        {
            return null;
        }
